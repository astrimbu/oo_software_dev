...
final class Record {
  ...
  Record(VideoObj video, int numOwned, int numOut, int numRentals) {
    if (video == null)
    	throw new IllegalArgumentException("Video must be non-null.");
    if (numOwned <= 0)
    	throw new IllegalArgumentException("numOwned must be > 0.");
    if (numOut > numOwned)
    	throw new IllegalArgumentException("numOut must be <= numOwned.");
    if (numRentals < numOut)
    	throw new IllegalArgumentException("numRentals must be >= numOut.");
    this.video = video;
